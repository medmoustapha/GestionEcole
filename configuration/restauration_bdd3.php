...
<br />
<p class="explic textgauche"><?php echo $Langue['LBL_RESTAURATION_EXPL8']; ?></p>
<ol class="explic textgauche">
<li><?php echo $Langue['LBL_RESTAURATION_EXPL9']; ?></li>
<li><?php echo $Langue['LBL_RESTAURATION_EXPL10']; ?></li>
</ol>
<br />
<br />
<div id="msg_ok"></div>
...
<input type="hidden" name="action" id="action" value="restauration_bdd4">
<?php echo $Langue['LBL_RESTAURATION_FICHIER_RESTAURE']; ?> : <input type="file" name="fichier" id="fichier" size=75>
    ...
    $("#form_restaure").submit(function(event)
    {
      if ($("#fichier").val()=="")
      {
        event.preventDefault();
	      $("#msg_ok").fadeIn( 1000 );
 		  $("#msg_ok").html('<div class="ui-widget" style="width:80%"><div class="ui-state-error ui-corner-all margin10_haut marge10_tout"><strong><?php echo $Langue['MSG_RESTAURATION_FICHIER']; ?></strong></div></div>');
   		  setTimeout(function()
          {
      ...
      else
      {
        ...
        else
        {
          event.preventDefault();
  	      $("#msg_ok").fadeIn( 1000 );
 	  	  $("#msg_ok").html('<div class="ui-widget" style="width:80%"><div class="ui-state-error ui-corner-all margin10_haut marge10_tout"><strong><?php echo $Langue['MSG_RESTAURATION_PAS_ZIP']; ?></strong></div></div>');
   	  	  setTimeout(function()
          {
            $("#msg_ok").effect("blind",1000);
          }, 3000 );
...
function formUploadCallback (result) {
  switch (result)
  {
    case "move":
      $("#msg_ok").fadeIn( 1000 );
		  $("#msg_ok").html('<div class="ui-widget" style="width:80%"><div class="ui-state-error ui-corner-all margin10_haut marge_tout"><strong><?php echo $Langue['ERR_MAJ_DEPLACEMENT']; ?></strong></div></div>');
 			setTimeout(function()
      {
        $("#msg_ok").effect("blind",1000);
      }, 6000 );
    ...
    case "upload":
      $("#msg_ok").fadeIn( 1000 );
		  $("#msg_ok").html('<div class="ui-widget" style="width:80%"><div class="ui-state-error ui-corner-all margin10_haut marge_tout"><strong><?php echo $Langue['ERR_MAJ_UPLOAD']; ?></strong></div></div>');
 			setTimeout(function()
      {
        $("#msg_ok").effect("blind",1000);
      }, 6000 );
    ...
    default:
      $("#msg_ok").html('');
      $("#button_restaurer").html('<input type="Button" id="etape4" name="etape4" value="<?php echo $Langue['BTN_RESTAURATION_ETAPE4']; ?>">');
      $("#etape4").button();
      $("#etape4").click(function()
      {
        Charge_Dialog("index2.php?module=configuration&action=restauration_bdd5&fichier="+result,"<?php echo $Langue['LBL_RESTAURATION_TITRE']; ?>");
      });
      break;
  }
}
