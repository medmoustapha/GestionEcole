...
<tr>
  <td class="textgauche marge5_droite" style="width:50%" valign=top>
    ...
    <tr>
      ...
      <td class="gauche" style="width:94%"><label class="label_detail_non_gras"><?php echo $Langue['LBL_REGISTRE_MOIS']; ?></label></td>
    </tr>
    <tr>
      <td class="droite" style="width:6%">&nbsp;</td>
      <td class="gauche" style="width:94%">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $Langue['LBL_IMPRESSION_MOIS']; ?> : <select name="id_mois" id="id_mois" class="text ui-widget-content ui-corner-all">
      <?php
	    if ($gestclasse_config_plus['etendue_annee_scolaire']=="1")
		{
			...
			{
			  echo '<option value="'.$i.'"';
			  if ($i==$_SESSION['mois_en_cours']) { echo ' SELECTED'; }
			  echo '>'.$liste_choix['mois'][$i].' '.$_SESSION['annee_scolaire'].'</option>';
			}
		}
		else
		{
			...
			$anneea=$_SESSION['annee_scolaire']+1;
			for ($i=1;$i<=$gestclasse_config_plus['mois_annee_scolaire'];$i++)
			{
			  echo '<option value="'.$i.'"';
			  if ($i==$_SESSION['mois_en_cours']) { echo ' SELECTED'; }
			  echo '>'.$liste_choix['mois'][$i].' '.$anneea.'</option>';
			}
		}
      ?>
      </select></td>
    ...
    <tr>
      ...
      <td class="gauche" style="width:94%"><label class="label_detail_non_gras"><?php echo $Langue['LBL_IMPRESSION_PAGE_GARDE']; ?></label></td>
    </tr>
    <tr>
      <td class="droite" style="width:6%"><input type="radio" id="document" name="document" value="3"></td>
      <td class="gauche" style="width:94%"><label class="label_detail_non_gras"><?php echo $Langue['LBL_IMPRESSION_STATISTIQUES']; ?></label></td>
    </tr>
    </table>
  </td>
...
<?php if ($_SESSION["type_util"]=="D") { ?>
		window.open("http://www.doxconception.com/site/index.php/directeur-registre-dappel/article/142-absences-documents-imprimables.html","Aide");
<?php } else { ?>
		window.open("http://www.doxconception.com/site/index.php/prof-registre-dappel/article/142-absences-documents-imprimables.html","Aide");
<?php } ?>
  });
  ...
  {
    data = "document="+$("#document:checked").val()+"&id_mois="+$("#id_mois").val()+"&orientation="+$("#orientation:checked").val()+"&couleur="+$("#couleur:checked").val()+"&numerotation="+$("#numerotation").val();
    window.open("index2.php?module=absences&action=imprimer_lancer&"+data,"Impression");
  ...
  $("#Annuler_Impression2").button();
	$("#Annuler_Impression").click(function(event)
  {
    $("#dialog-form").dialog( "close" );
  });
	$("#Annuler_Impression2").click(function(event)
  {
    $("#dialog-form").dialog( "close" );
  });
});
