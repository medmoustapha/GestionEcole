...
// Récupération des informations
  ...
  $tpl->set_file("gform","detailview_transfert.html");
  ...
  foreach ($tableau_variable['cooperative'] AS $cle)
  {
    if ($cle['nom']!="ligne_comptable")
	{
      $tpl->set_var(strtoupper($cle['nom']), Variables_Affiche($cle));
	}
  }
  
  $tpl->set_var('BANQUE2',$liste_choix['cooperative_banque'][mysql_result($req,0,'ligne_comptable')]); 
  $tpl->set_var('MONTANT',number_format(Abs(mysql_result($req,0,'montant')),2,',',' ').' &euro;'); 
...
$(document).ready(function()
{
  ...
  {
    Charge_Dialog("index2.php?module=cooperative&action=editview_transfert&id=<?php echo $_GET['id']; ?>","<?php echo $Langue['LBL_SAISIE_MODIFIER_TRANSFERT']; ?>");
  });
