...
// Récupération des informations
  ...
  $req = mysql_query("SELECT * FROM `eleves` WHERE id = '" . $_SESSION['id_util'] . "'");
  foreach ($tableau_variable['eleves'] AS $cle)
  {
    if ($cle['nom']!="passe2")
    {
  ...
  $tableau_variable['eleves']['passe2']['value'] = "";

  $tpl = new template("compte");
  $tpl->set_file("gform","editview_E.html");
  ...
  $req2=mysql_query("SELECT * FROM `contacts_eleves` WHERE id_eleve='".$_SESSION['id_util']."' ORDER BY nom ASC");
	$tabindex=29;
...
$(document).ready(function()
{
  ...
  {
    Charge_Dialog3("index2.php?module=compte&action=saisir_photo1&type=E&id_personne=<?php echo $_SESSION['id_util']; ?>","<?php echo $Langue['LBL_PHOTO_TITRE']; ?>");
  });
  ...
  $("#form_editview").submit(function(event)
  {
    ...
    if ( bValid )
    {
      updateTips("save","compte","<?php echo $Langue['LBL_FICHE_ELEVE']; ?>");
      var $form = $( this );
