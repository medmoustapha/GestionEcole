...
<input type="hidden" name="module" id="module" value="compte">
...
<?php
  ...
  else
  {
	...
	{
		$req=mysql_query("SELECT * FROM `eleves` WHERE id='$id_personne'");
...
<?php if ($_SESSION["type_util"]=="P") { ?>
		window.open("http://www.doxconception.com/site/index.php/prof-votre-compte.html","Aide");
<?php } else { ?>
		window.open("http://www.doxconception.com/site/index.php/parent-votre-compte.html","Aide");
<?php } ?>
  });
  ...
  {
    Charge_Dialog3("index2.php?module=compte&action=saisir_photo_webcam&id_personne=<?php echo $_GET['id_personne']; ?>","<?php echo $Langue['BTN_PHOTO_WEBCAM']; ?>");
  });
	  ...
	  buttons:[
		{
			...
			click: function()
			{
				...
				var request = $.ajax({type: "POST", url: "index2.php", data: "module=compte&action=delete_photo&id=<?php echo $_GET['id_personne']; ?>"});
				request.done(function(msg)
				{
					Message_Chargement(1,1);
					$( "#dialog-confirm" ).dialog( "close" );
					Charge_Dialog3("index2.php?module=compte&action=saisir_photo1&id_personne=<?php echo $_GET['id_personne']; ?>","<?php echo $Langue['LBL_PHOTO_TITRE']; ?>");
...
function formUploadCallback (result) {
  switch (result)
  {
    ...
    case "upload":
      ...
      break;
    default:
      Charge_Dialog3("index2.php?module=compte&action=saisir_photo3&id_personne=<?php echo $_GET['id_personne']; ?>","<?php echo $Langue['LBL_PHOTO_TITRE']; ?>");
      break;
  }
}
</script>
