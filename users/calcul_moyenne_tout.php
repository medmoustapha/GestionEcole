<?php
  ...
  $req_classe=mysql_query("SELECT classes.*,classes_niveaux.*,classes_profs.* FROM `classes`,`classes_niveaux`,`classes_profs` WHERE classes.id=classes_profs.id_classe AND classes_profs.id_prof='$id_titulaire' AND classes_profs.type='T' AND classes_profs.id_classe=classes_niveaux.id_classe AND classes.annee='$annee'");
  for ($pli=1;$pli<=mysql_num_rows($req_classe);$pli++)
  {
    $id_classe=mysql_result($req_classe,$pli-1,'classes.id');
    $id_niveau=mysql_result($req_classe,$pli-1,'classes_niveaux.id_niveau');
...
for ($poi=1;$poi<=$oli;$poi++)
{
  ...
  for ($z=1;$z<=mysql_num_rows($req_eleve);$z++)
  {
    ...
    for ($i=$max;$i>=0;$i--)
    {
      ...
      foreach ($liste_choix['matieres_stat'] AS $cle => $value)
      {
		...
		{ 
		  $total_classe_stat[$cle][$i]=$total_classe_stat[$cle][$i]+(100*$total_result_stat[$cle][$i]/$total_periode_stat[$cle]); 
		}
	  }
    }
  }
  ...
  for ($i=$max;$i>=0;$i--)
  {
    ...
    {
      $req=mysql_query("INSERT INTO `moyenne_classe` (id_classe,id_niveau,marquage,statistiques) VALUES ('$id_classe','$id_niveau','$i')");
    }
    foreach ($liste_choix['matieres_stat'] AS $cle => $value)
    {
  ...
  for ($i=$max;$i>=0;$i--)
  {
    ...
    foreach ($liste_choix['matieres_stat'] AS $cle => $value)
    {
      ...
      {
        $moy=$total_classe_stat[$cle][$i]/$nb_eleve_stat[$cle];
