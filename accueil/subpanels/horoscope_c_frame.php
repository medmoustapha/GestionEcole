<?php
  ...
  $rss = fetch_rss("http://www.asiaflash.com/horoscope/rss_horochinjour_".$signe.".xml");
   ...
   foreach ($items as $item)
   {
	 ...
	 foreach ($liste_choix['horoscope_c'] AS $cle => $value)
	 {
	   ...
	   if ($i<=9) { $j="0".$i; } else { $j=$i; }
  	   $description=str_replace('<br/><center><br/><img src="http://www.asiaflash.com/anh/pjour'.$j.'.gif" alt="'.$value.'" title="'.$value.'"/></center>','',$description);
