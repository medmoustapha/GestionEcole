...
$req=mysql_query("SELECT * FROM `competences` WHERE id_prof='".$_SESSION['titulaire_classe_cours']."' AND id_niveau='".$_SESSION['niveau_en_cours']."' AND cree<='$annee_fin' AND (supprime='0000-00-00' OR supprime>='$annee_fin') AND id_cat='$id_cat' ORDER BY ordre ASC");
for ($i=1;$i<=mysql_num_rows($req);$i++)
{
  $id=mysql_result($req,$i-1,'id');
  $ordre=mysql_result($req,$i-1,'ordre');
  $key=array_search($ordre,$_POST['ListeCompetence'.$id_cat])+1;
  $req2=mysql_query("UPDATE `competences` SET ordre='$key' WHERE id='$id'");
}
?>